...
stamps_ind(find(stamps_ind(:,1)>0))=[]; %changed here This change is not reflected in dataL I would maybe add somethign like dataL(find(stamps_ind(:,1)>0),:)=nan to make sure the rows were the index is deleted is also deleted from the data
