...
if ~isempty(cData)
    ...
    lastColorPlot = app.getLastColorPlot();
    firstScanIndex = 1;
    lastScanIndex = size(cData,2);
    firstScanTime = firstScanIndex / sampling_freq;
    lastScanTime = lastScanIndex / sampling_freq;

    lastColorPlot.XTick = [firstScanIndex, lastScanIndex];
    lastColorPlot.XTickLabel = num2str([firstScanTime; lastScanTime]);
    ...
    if (app.getNumOfChannels() == 4)
        ...
        anodalLimit = app.getAnodalLimit();
        cathodalLimit = app.getCathodalLimit();
        axPlot.YTickLabel = [cathodalLimit; anodalLimit; cathodalLimit];
    end    
