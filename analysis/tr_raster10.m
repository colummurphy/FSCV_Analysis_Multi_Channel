...
%datavar='fscv'; %Processed variable (struct) tclim([20 50])o open in trlists.processed
...
if strcmp(signaltype,'fscv')
    ...
    if (convertCharsToStrings(eventaln(1,:)) == 'reward_big' | convertCharsToStrings(eventaln(1,:)) == 'reward_small');
        for it=1:size(tbt_Data,1)
            bgwinids=(alnFsamps(it)-bgwinSampSize+100):(alnFsamps(it)+100-1); % gets the next display fix after reward (10s from reward)
            bgwinids(bgwinids<1)=[];
            bgwinids(bgwinids>size(tbt_Data,2))=[];
            if isempty(bgwinids)
                STOP;%Call debug functioned
                error(['bg ids bed' ...
                    'ond boundaries of data']);
            end
            bgmean=mean(tbt_Data(it,bgwinids),'omitnan');        %Mean background signal around aln evt
    ...
    if strcmp(signaltype,'nlx') && contains(nlxname,'eyed')
        ...
        ed_data= tbt_Data;
        blink_thres=3.5e-3;%Absolute peak threshold to remove eye blinks (usually hits rail)
        blink_pad=70;%Samples for padding around blink removal NEED TO BE WORKED ON FOR BETTER BLINKS REMOVAL
        for it=1:size(tbt_Data,1)
            tbt_Data(it,:)=deglitchnanamp(tbt_Data(it,:),blink_thres,blink_pad);
        end
    end
    ...
    %% Background subtract FSCV data when aligned to reward
%     if convertCharsToStrings(eventaln(1,:)) == 'reward_big';
%         for it=1:size(tbt_Data,1)
%             bgwinids=(alnFsamps(it)+100)-(bgwinSampSize:alnFsamps(it)+100-1); % gets the next display fix after reward (10s from reward)
%             bgwinids(bgwinids<1)=[];
%             bgwinids(bgwinids>size(tbt_Data,2))=[];
%             if isempty(bgwinids)
%                 STOP;%Call debug functioned
...
%             tbt_Data(it,:)=tbt_Data(it,:)-bgmean;   %Subtract mean background signal
%         end
%     elseif convertCharsToStrings(eventaln(1,:)) == 'reward_small';
%         for it=1:size(tbt_winData,1)
%             bgwinids=(alnFsamps(it)+100)-(bgwinSampSize:alnFsamps(it)+100-1); % gets the next display fix after reward (10s from reward)
%             bgwinids(bgwinids<1)=[];
%             bgwinids(bgwinids>size(tbt_Data,2))=[];
%             if isempty(bgwinids)
